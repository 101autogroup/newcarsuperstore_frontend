...
export default function LeadFormButton({
  ...
  async function handleContinue() {
      ...
      const url = new URL(
              buildLeadFormUrl({ vin, make, model, trim, year, source })
            );
      ...
      // Fire Google Ads conversion event via GTM dataLayer
      if (typeof window !== "undefined" && window.dataLayer) {
              window.dataLayer.push({
                        event: "lead_form_submitted",
                        lead_source: source ?? "get_price",
                ...
                {leadUrl ? (
                    <>
                                <DialogHeader className="border-b border-ink-200 px-5 py-3">
                                              <DialogTitle className="text-base">{title}</DialogTitle>DialogTitle>
                    ...
                    <>
                                <DialogHeader className="border-b border-ink-200 px-5 py-3">
                                              <DialogTitle className="text-base">Tell us about your request</DialogTitle>DialogTitle>
                                </DialogHeader>DialogHeader>
                                <div className="grid gap-3 p-5">
                                  {vehicleLabel && (
                                      <p className="rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm text-ink-700">
                                                        Car: <span className="font-medium">{vehicleLabel}</span>span>
                                      </p>p>
                                              )}
